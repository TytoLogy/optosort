function varargout = plotrefdata( data, winLen, varargin )
% modified version of MultiChanPlot. 
% Press left and right arrow to jump forward and back. Add shift to move by
% 1/10th of a window. Press up and down to change the y-axis scale by 10%.
% Press 'c' to set the plotted channels. Press 'y' to set the y-axis range.
% Press 'l' to set the window duration. Press 'p' to set a new current
% position. Press 'x' to delete all manually-marked intervals and reset
% pre-loaded intervals. Press 'h' to open a help window with a list of
% commands. Marking intervals: Press the left mouse button to start marking
% an interval, and the right mouse button to finish marking an interval.
% Clicking the left mouse button within a marked interval will cancel it.
% Ending a marked interval within an already  marked interval will
% concatenate the two intervals. Intervals can be set across different
% display windows (press left mouse button, then left or right keys, then
% right mouse button). Marked intervals which have been pre-loaded into the
% function will appear in a different color, but behave identically to
% manually marked intervals.
%
% Usage: 
% MultiChanPlot(data,winLen):                Displays a multiple-channel
%                                            plot, where "data" is a time x
%                                            channels matrix and "winLen"
%                                            is the duration of the shown
%                                            window.
% MultiChanPlot(...,'ylim',[ymin ymax]):     Sets the y-scale of each
%                                            channel, where YL is a 2-element
%                                            vector. Default value is the
%                                            minimum and maximum values of
%                                            the input dataset.
% MultiChanPlot(...,'channels',c):           Selects which channels will be
%                                            plotted. 
% MultiChanPlot(...,'srate',sr):             Sets the sampling
%                                            rate of the input data. The "winLen"
%                                            variable will be interpreted
%                                            as seconds. Default value is
%                                            1.
% MultiChanPlot(...,'maskdata',m):           Provides an initial set of
%                                            marked intervals, where m is a N x 1
%                                            vector with the same length as
%                                            the input data, with  non-zero
%                                            samples corresponding to
%                                            marked intervals.
% mask = MultiChanPlot(...)                  Setting an output variable
%                                            will cause the marked segments to
%                                            be exported in the form of a
%                                            Nx1 boolean vector.
% 
